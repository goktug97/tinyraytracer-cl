...
(defclass model (object)
  ((model-path
   ...
   (bbox
    :reader model-bbox)
   (edge1
    :reader intersect-edge1)
   (edge2
    :reader intersect-edge2)))
	   ...
	   (let* ((tvec (m- ray-origin (elt face 0)))
		 ...
		 (unless (or (< v 0) (> (+ u v) det))
		   (let* ((tnear (* (dot-product edge2 qvec) (/ 1f0 det))))
		     (when (> tnear 1e-5)
		       (setf (slot-value model 'edge1) edge1
			     (slot-value model 'edge2) edge2)
		       (return tnear)))))))))))

(defmethod object-normal ((ray ray) (model model) t0)
  (let* ((hit (m+ (ray-origin ray) (.* (ray-direction ray) t0)))
	 (normal (normalize (cross (intersect-edge1 model) (intersect-edge2 model)))))
    (list hit normal)))
